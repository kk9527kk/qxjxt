...
<table border="0" id="table1"  align="center" cellspacing="0" cellpadding="0" width="750" height="47">
	<tr>
		<td width="748" height="35">
          <p align="center"><font color="#0000FF" face="黑体" size="6">计算机耗材及配件申请待审批事项</font>
          <p align="center">　
        </td>
	</tr>
</table>
<div align="left">
    <table border="0" width="745"  align="center" cellspacing="1" cellpadding="0" bgcolor="#000000">
      <tr> 
        ...
        <td width="80" bgcolor="#FFFFFF" height="23" background="images/bg1.gif" align="center">
        型号</td>
		<td width="45" bgcolor="#FFFFFF" height="23" background="images/bg1.gif" align="center">
        数量</td>

        <td width="62" bgcolor="#FFFFFF" height="23" background="images/bg1.gif" align="center">
        <font size="2">申请日期</a></font></td>

       
        <td width="66" bgcolor="#FFFFFF" height="23" background="images/bg1.gif" align="center">审批处理</td>
       
      </tr>
      <%dim mc
    sql="select *  from SQSPB WHERE ZT='2'  order by SQRQ,id "
    set rs=server.createobject("adodb.recordset")
    rs.open sql,connstr,1,1
    if rs.eof then
      %>
      <tr> 
        <td bgcolor="#FFFFFF" colspan="8" width="419"> 
...
%>
      <tr> 
        <td width="50" bgcolor="#FFFFFF" height="20" align="center"><%=rs("dw")%></a></td>
        <td width="60" bgcolor="#FFFFFF" height="20" align="center"><%=rs("sqr")%></a></td>
        <td width="60" bgcolor="#FFFFFF" height="20" align="center"><%=rs("syr")%>　</td>
        <td bgcolor="#FFFFFF" height="20" align="center" width="98"><%=rs("hcmc")%></a></td>
        <td bgcolor="#FFFFFF" height="20" align="center" width="80"><%=rs("xh")%></td>
        <td bgcolor="#FFFFFF" height="20" align="center" width="45"><%=rs("sl")%></td>
        <td width="62" bgcolor="#FFFFFF" height="20" align="center"><%=rs("sqrq")%></td>
      
        <td width="66" bgcolor="#FFFFFF" height="20" align="center"><a href="spzx.asp?id=<%=rs("id")%>"><font color="#FF0000">审批</font></a>|<a href="del.asp?id=<%=rs("id")%>"><font color="#00FFFF">删除</font></a></td>
...
<div align="left">
  <table border="0" width="745" align="center"  cellspacing="0" cellpadding="0">
    ...
    <td width="464">页数：<%=currentpage%>/<% =n%>
   	<%k=currentpage                                                                
   	if k<>1 then%>
   	[<a href="sp1.asp?pageid=1">首页</a>]                                                                
   	[<a href="sp1.asp?pageid=<%=k-1%>">上一页</a>]                                                                
   	<%else%>
   	[首页]&nbsp;[上一页]                                                                
   	<%end if%>
   	<%if k<>n then%>                                                                
   	[<a href="sp1.asp?pageid=<%=k+1%>">下一页</a>]                                                                
   	[<a href="sp1.asp?pageid=<%=n%>">尾页</a>]                                                                
   	<%else%>
   	[下一页]&nbsp;[尾页]                                                                
   	<%end if%>
      ...
      <td height="23" width="277">  
        ...
        <select onChange="pagei()" size="1" name="pag" style="background-attachment: scroll; background-repeat: repeat; font-size: 9pt; height: 16; width: 77; background-color: #FFFFFF; border: 1 solid #000000; background-position: none 0%">  
          ...
          do while not i > n
          %>
          <option value="sp1.asp?pageid=<%=i%>" <%if i=currentpage then %>selected<%end if%>>第 
          <%=i%> 页</option>
          <%
          i=i+1
          loop
