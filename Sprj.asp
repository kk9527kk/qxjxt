...
<div align="left">
    <table border="0" width="772"  align="center" cellspacing="1" cellpadding="0" bgcolor="#000000">
      ...
      <%dim mc
    sql="select *  from QXJDJB WHERE QJZT='2'  order by SQSJ"
...
%>
      <tr> 
        ...
        <td width="60" bgcolor="#FFFFFF" height="20" align="center"><a href="sprjk.asp?id=<%=rs("id")%>"><font color="#FF0000">审批</font></a></td>
...
<div align="left">
  <table border="0" width="771" align="center"  cellspacing="0" cellpadding="0">
    ...
    <td width="464">页数：<%=currentpage%>/<% =n%>
   	<%k=currentpage                                                    
   	if k<>1 then%>
   	[<a href="sprj.asp?pageid=1">首页</a>]                                                    
   	[<a href="sprj.asp?pageid=<%=k-1%>">上一页</a>]                                                    
   	<%else%>
   	[首页]&nbsp;[上一页]                                                    
   	<%end if%>
   	<%if k<>n then%>                                                    
   	[<a href="sprj.asp?pageid=<%=k+1%>">下一页</a>]                                                    
   	[<a href="sprj.asp?pageid=<%=n%>">尾页</a>]                                                    
   	<%else%>
   	[下一页]&nbsp;[尾页]                                                    
   	<%end if%>
        共有<font color="red"><%=totalput%></font>条记录                                                   
      <b><font color="#0000FF"><font size="2">&nbsp;</font></font></b></td> 
          <form action="sprj.asp" name="pfrm">     
      <td height="23" width="303">  
        ...
        <select onChange="pagei()" size="1" name="pag" style="background-attachment: scroll; background-repeat: repeat; font-size: 9pt; height: 16; width: 77; background-color: #FFFFFF; border: 1 solid #000000; background-position: none 0%">  
          ...
          do while not i > n
          %>
          <option value="sprj.asp?pageid=<%=i%>" <%if i=currentpage then %>selected<%end if%>>第 
          <%=i%> 页</option>
          <%
          i=i+1
          loop
