 ...
 %>
  ...
  <tr>
    <td width="464">页数：<%=currentpage%>
      ...
      <%k=currentpage                                                                                                                                                                                 
   	if k<>1 then%>
      [<a href="qjtycxcl.asp?pageid=1">首页</a>] [<a href="qjtycxcl.asp?pageid=<%=k-1%>">上一页</a>]                                                                                                                   
      <%else%>
      [首页]&nbsp;[上一页] <%end if%>                                                                                                                  
      <%if k<>n then%>                                                                                                                  
      [<a href="qjtycxcl.asp?pageid=<%=k+1%>">下一页</a>] [<a href="qjtycxcl.asp?pageid=<%=n%>">尾页</a>]                                                                                                                   
      <%else%>
      [下一页]&nbsp;[尾页] <%end if%>                                                                                                                  
    ...
    <td height="23" width="302">
      ...
      <p align="right"><select onChange="pagei()" size="1" name="pag" style="background-attachment: scroll; background-repeat: repeat; font-size: 9pt; height: 16; width: 77; background-color: #FFFFFF; border: 1 solid #000000; background-position: none 0%">
        <%                                                                                                                                                           
          ...
          do while not i > n
          %>
        <option value="qjtycxcl.asp?pageid=<%=i%>" <%if i=currentpage then %>selected<%end if%>>第 
        <%=i%> 页</option>
        <%
          i=i+1
          loop
