...
if not (isdate(rqq) and  isdate(rqz)) then
   response.write "<script language=JavaScript>{window.alert('请输入正确的日期格式!');window.location.href='qjcx.asp';}</script>"
   response.end
end if 

    
if DateDiff("d",rqq,rqz)<0 then 
   response.write "<script language=JavaScript>{window.alert('请输入起时间大于输入时间止时间！');window.location.href='qjcx.asp';}</script>"
   response.end
end if
