...
<div align="left">
    <table border="0" width="761"  align="center" cellspacing="1" cellpadding="0" bgcolor="#000000">
      ...
      <%dim mc
    sql="select *  from QXJDJB WHERE (QJZT='5' OR QJZT='6' OR QJZT='7') AND ZGZT='2' order by SQSJ "
...
%>
      <tr> 
       ...
       <form method="POST" action="CXXJCL.asp?id=<%=rs("id")%>" name="form1">
           <input type="text" name="SXTS" size="7" value="<%=rs("SXTS")%>">　
          </td>
       
        <td width="95" bgcolor="#FFFFFF" height="20" align="center" valign="middle"><font color="#FF0000"><input type="submit" value="重新销假" name="B1"></font></form></td>
...
<div align="left">
  <table border="0" width="762" align="center"  cellspacing="0" cellpadding="0">
    ...
    <td width="464">页数：<%=currentpage%>/<% =n%>
   	<%k=currentpage                                                                                  
   	if k<>1 then%>
   	[<a href="CXXJ.asp?pageid=1">首页</a>]                                                                                  
   	[<a href="CXXJ.asp?pageid=<%=k-1%>">上一页</a>]                                                                                  
   	<%else%>
   	[首页]&nbsp;[上一页]                                                                                  
   	<%end if%>
   	<%if k<>n then%>                                                                                  
   	[<a href="CXXJ.asp?pageid=<%=k+1%>">下一页</a>]                                                                                  
   	[<a href="CXXJ.asp?pageid=<%=n%>">尾页</a>]                                                                                  
   	<%else%>
   	[下一页]&nbsp;[尾页]                                                                                  
   	<%end if%>
      ...
      <td height="23" width="294">  
        ...
        <select onChange="pagei()" size="1" name="pag" style="background-attachment: scroll; background-repeat: repeat; font-size: 9pt; height: 16; width: 77; background-color: #FFFFFF; border: 1 solid #000000; background-position: none 0%">  
          ...
          do while not i > n
          %>
          <option value="CXXJ.asp?pageid=<%=i%>" <%if i=currentpage then %>selected<%end if%>>第 
          <%=i%> 页</option>
          <%
          i=i+1
          loop
